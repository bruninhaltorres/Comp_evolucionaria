...
# Função para gerar uma taxa de mutação variável para cada cromossomo
def gerar_taxas_mutacao(populacao):
    taxas = []
    for i in range(len(populacao)):
        taxa = random.uniform(0.01, 0.05)  # taxa de mutação entre 1% e 5%
        taxas.append(taxa)
    return taxas
...
for geracao in range(num_geracoes):
    ...
    taxas_mutacao = gerar_taxas_mutacao(populacao)  # gerar lista de taxas de mutação
    # for filho in filhos:
    #     if random.random() < taxa_mutacao:
    #         filho[0] += random.uniform(-1, 1)
    #         filho[1] += random.uniform(-1, 1)
    for i, filho in enumerate(filhos):
      if random.random() < taxas_mutacao[i]:
        filho[0] += random.uniform(-1, 1)
        filho[1] += random.uniform(-1, 1)
